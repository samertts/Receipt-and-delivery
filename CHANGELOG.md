...
* ترحيل مصادقة واجهة الويب إلى HttpOnly access/refresh Cookies مع SameSite وSecure في الإنتاج، وCSRF token/header للطلبات المغيرة.
* إزالة فك JWT وتخزينه وإرساله من JavaScript، واستعادة المستخدم عبر `/api/auth/me`، مع إبقاء `/api/v1` كتوافق انتقالي لعملاء Bearer غير المتصفحية.
* تأمين WebSocket المتصفح باستخدام access cookie وفحص Origin، ومنع الرموز في query string أو رسالة المصادقة.
* إضافة توثيق طباعة الوصل ونموذج PDF تجريبي يوضح شكل الإيصال.
* إضافة تخصيص هوية الإيصال محليًا: اسم الشركة والعنوان والتذييل والشعار المحلي واللون الرئيسي مع تحقق آمن وكتابة ذرية.
* إضافة تخطيطات A5 ونسختين A5 جنبًا إلى جنب على A4 في تطبيق سطح المكتب، مع اختيار التخطيط من نافذة تفاصيل الإيصال.
* إضافة اختيار A4/A5/نسختين على A4 إلى طباعة الويب مع تسميات نسخة المستلم ونسخة المرسل.
* إضافة عرض وقت التسجيل وآخر تحديث بصيغة زمنية تفصيلية دون افتراض منطقة زمنية غير محفوظة.
* إضافة formatter مركزي لرسائل الأخطاء العربية الآمنة ومنع عرض الاستثناءات الخام في مسار إعدادات وطباعة سطح المكتب.
* إضافة زر طباعة وصل من تفاصيل المعاملة في واجهة الويب بقالب منظم يدعم العربية والإنجليزية.
...
* إصلاح تمرير كامل بيانات المعاملة والبنود والتفويض والتعليقات إلى إيصال PDF في تطبيق سطح المكتب.
* إصلاح تشكيل واتجاه النص العربي في إيصال PDF باستخدام Arabic reshaper وBiDi.
