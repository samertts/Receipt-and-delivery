...
def upsert_organization(payload, user=None):
    with _db.get_conn() as conn:
        if payload.get("id"):
            conn.execute(
                """UPDATE organizations SET name=?,code=?,org_type=?,governorate=?,address=?,phone=?,email=?,logo_path=?,notes=?,status=? WHERE id=?""",
                (
                    payload["name"],
                    payload["code"],
                    payload["org_type"],
            ...
            conn.execute(
                """INSERT INTO organizations(name,code,org_type,governorate,address,phone,email,logo_path,notes,status) VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (
                    payload["name"],
                    payload["code"],
                    payload["org_type"],
