...
def seed_default_users():
    ...
    if created:
        ...
        log_action(
            None, "admin_seeded", "Default admin account created on first startup"
        )
...
def authenticate(username, password, max_attempts=5, lockout_minutes=5):
    ...
    with _db.get_conn() as conn:
        ...
        if fail_row and fail_row["c"] >= max_attempts:
            ...
            conn.execute(
                "INSERT INTO audit_logs(user_id,action,machine_name,timestamp,details) VALUES(?,?,?,?,?)",
                (
                    None,
                    "login_failed",
                    platform.node(),
                    now,
                    f"فشل تسجيل الدخول للمستخدم: {username}",
                ),
            )
...
def list_users():
    with _db.get_conn() as conn:
        rows = conn.execute(
            "SELECT u.id, u.full_name, u.username, u.role, u.institution_id, u.phone, u.notes, u.status, u.password_changed_at, o.name institution_name FROM users u LEFT JOIN organizations o ON o.id=u.institution_id ORDER BY u.id DESC"
        ).fetchall()
