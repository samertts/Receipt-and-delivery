...
def setup_test_db():
    ...
    conn.execute(
        "INSERT INTO receipts(receipt_no, tx_type_id, sender_org_id, receiver_org_id, sender_name, receiver_name, created_at, status, created_by) VALUES('R-001',1,1,1,'Sender','Receiver','2024-01-01T00:00:00','Draft',1)"
    )
    conn.execute(
    ...
    conn.execute(
        "INSERT INTO attachments(receipt_id, file_path, file_type, file_hash, file_size, category, created_at) VALUES(1,'/nonexistent/file.pdf','pdf','hash',1000,'doc','2024-01-01T00:00:00')"
    )
    conn.execute(
        "INSERT INTO sync_queue(entity_type, entity_id, action, created_at) VALUES('receipt',1,'create','2024-01-01T00:00:00')"
    )
    conn.execute(
        "INSERT INTO login_attempts(username, success, attempted_at) VALUES('admin',1,'2024-01-01T00:00:00')"
    )
    conn.commit()
    conn.close()
...
try:
    ...
    ok(
        result.get("error") == "File not found",
        "verify_backup error is 'File not found'",
    )
finally:
...
conn.execute("PRAGMA foreign_keys = ON;")
conn.execute(
    "INSERT OR REPLACE INTO migration_lock(id, is_locked, owner, updated_at) VALUES(1,1,'failed_process',?)",
...
try:
    ...
    ok(
        "too small" in result.get("error", "").lower(),
        "partial backup has descriptive error",
    )
