...
# Check excludes
excluded = ['tkinter', 'matplotlib', 'numpy', 'scipy', 'pandas', 'cv2', 'transformers', 'torch', 'tensorflow']
for ex in excluded:
    ok(f"'{ex}'" in spec_text, f"  excluded: {ex}")

# Check executable name
ok("name='LabReceiptSystem'" in spec_text or 'name="LabReceiptSystem"' in spec_text,
...
required_dirs = ['database', 'attachments', 'backups', 'logs', 'exports', 'settings',
                 'templates', 'recovery', 'diagnostics', 'migrations', 'updates', 'receipts']
for d in required_dirs:
    ok(d in iss_text, f"  [Dirs] includes '{d}'")

# Check version sync
ok("#define MyAppVersion" in iss_text, "  version defined as #define MyAppVersion")
ok("AppVersion={#MyAppVersion}" in iss_text, "  AppVersion references #define (not hardcoded)")

# Check uninstall preserves user data
ok("[UninstallDelete]" in iss_text, "  [UninstallDelete] section exists")
ok("temp" in iss_text.split("[UninstallDelete]")[1].split("[UninstallRun]")[0],
   "  temp directory is deleted on uninstall")
ok("[UninstallRun]" in iss_text, "  [UninstallRun] section exists")

# Check main EXE is bundled
ok("dist\\{#MyAppExeName}" in iss_text or "dist\\LabReceiptSystem.exe" in iss_text,
...
required_dirs2 = ['database', 'attachments', 'backups', 'logs', 'exports', 'settings',
                  'templates', 'recovery', 'diagnostics', 'migrations', 'updates']
...
ok("dist/LabReceiptSystem.exe" in ci_text or "dist\\LabReceiptSystem.exe" in ci_text,
   "  EXE artifact is uploaded")
ok("installer/Output/LabReceiptSetup.exe" in ci_text or "installer\\Output\\LabReceiptSetup.exe" in ci_text,
   "  Installer artifact is uploaded")

# Check version synchronization in CI
ok("for /f %%i in (VERSION) do set APP_VER=%%i" in ci_text or "APP_VER" in ci_text,
...
required_pkgs = ['PySide6', 'reportlab', 'bcrypt', 'qrcode', 'python-barcode', 'Pillow', 'openpyxl']
